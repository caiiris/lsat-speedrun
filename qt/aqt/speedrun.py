...
TRAP_DISPLAY_NAMES: dict[str, str] = {
    ...
    "trap::false-dilemma": "False dilemma",
    "trap::appeal-to-authority": "Appeal to authority",
    ...
    "trap::negation-error": "Negation error",
    "trap::ambiguity": "Ambiguity",
    "trap::missing-link": "Missing link",
}
...
def _rail_tip_html(fields: dict[str, str]) -> str:
    ...
    skill_short = type_tag.replace("type::", "").replace("-", " ").title() if type_tag else "question-type"
    return (
        ...
        f"Strong test-takers predict the gap before reading the choices \u2014 it stops you from rationalizing a wrong answer."
        f"</div>"
        f'<div style="color:#aaa;font-size:0.76em;margin-top:8px;">(tips fade as your {_esc(skill_short)} score rises)</div>'
        f"</div>"
    )
...
def build_prephrase_html(fields: dict[str, str]) -> str:
    ...
    type_html = (
        f'<div class="sr-type-tag">{type_tag}</div>\n' if type_tag else ""
    )
    ...
    prephrase_box = (
        ...
        f"In one line, what must the right answer do?</div>\n"
        f'<input id="sr-prephrase-input" type="text" autocomplete="off" '
        f'placeholder="e.g. connect the premise to the conclusion\u2026" '
    ...
    rail = (
        ...
        f'<div class="sr-rail-card">{_rail_tags_html(fields)}</div>\n'
        f"</div>\n"
    )
...
def build_choices_html(fields: dict[str, str]) -> str:
    ...
    type_html = f'<div class="sr-type-tag">{type_tag}</div>\n' if type_tag else ""
    ...
    for label in _CHOICE_LABELS:
        ...
        if trap_tag and not is_this_correct:
            ...
            why_html += (
                ...
                f'border-radius:3px;color:{_C_AMBER};font-weight:600;">{_esc(trap_label_text)}</code></div>\n'
            )

        # Name-the-trap chips (only for committed wrong choice)
        trap_chips_html = ""
        if is_this_committed and show_name_trap:
            trap_chips_html = _build_trap_chips_html(item_traps, trap_chosen, trap_result)
...
def build_item_answer_html(fields: dict[str, str], committed: str) -> str:
    return build_reveal_html(fields, committed)
...
def _build_trap_chips_html(
    ...
    header = (
        ...
        f"Why did this choice fool you?</div>\n"
    )
    ...
    for trap_tag in item_traps:
        ...
        if trap_chosen is not None:
            ...
            chips += (
                f'<span class="sr-chip{css_extra}" '
    ...
    if trap_chosen is not None and trap_result is not None:
        if trap_result == "correct":
            result_msg = (
                ...
                f"\u2713 Correct \u2014 that\u2019s the trap on this choice.</div>\n"
            )
        else:
            result_msg = (
                f'<div style="font-size:0.82em;color:{_C_CLAY};margin-top:8px;font-weight:600;">'
                f"\u2717 Not quite \u2014 review the trap label on this choice.</div>\n"
            )
